...
async function fingerprintUntrackedPath(
  ...
  try {
    ...
    // Node does not expose O_NOFOLLOW on Windows. Keep the same fail-closed
    // intent by requiring a stable ordinary-file lstat plus an unchanged
    // canonical path before/after the read. POSIX keeps the stronger native
    // O_NOFOLLOW open semantics.
    const canonicalBefore = noFollow ? undefined : await realpath(absolutePath);
    if (canonicalBefore !== undefined && !sameFilesystemPath(canonicalBefore, absolutePath)) failPatch();

    const handle = await open(
      absolutePath,
      constants.O_RDONLY | (noFollow ? constants.O_NOFOLLOW : 0)
    );
    try {
      ...
      if (stableFileMetadata(openedAfter) !== metadata ||
          stableFileMetadata(pathAfter) !== metadata ||
          pathAfter.isSymbolicLink()) failPatch();
      if (canonicalBefore !== undefined) {
        const canonicalAfter = await realpath(absolutePath);
        if (!sameFilesystemPath(canonicalAfter, canonicalBefore)) failPatch();
      }
