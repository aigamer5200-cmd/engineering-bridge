...
Codex single-entry and recovery rule: while the durable executor mode is codex, every formal bounded Codex task must enter through Engineering Bridge. Do not use DS exec_command, PowerShell, CMD, or another direct shell invocation to carry the real task instruction to Codex. Diagnostic version/path/resolver checks are allowed, but they are not formal execution. A stale/unknown Bridge task, stdin/EOF or wrapper stall, internal multi-agent/memory stall, Windows sandbox/SID/Git-ownership friction, or repository write-authority conflict is a task-local recoverable failure when the provider itself remains healthy. If a safe retry/fallback can keep user-visible forward progress continuous, discard only the stale task/thread/child, keep executor_mode=codex, preserve healthy sibling tasks, and start a fresh bounded Bridge task without Telegram or a Human Gate. Emit the Telegram interruption notice only when forward execution actually stops or yields before recovery.

Repository mutation authority is project-local. For ordinary isolated-worktree Lane A/B work, DS apply_patch is the preferred mutation lane unless the target project explicitly authorizes Bridge controlled writes. Keep Codex read-only and use Bridge to produce/refine a bounded patch proposal when needed; DS applies the reviewed patch and independently audits/regresses it. A Bridge APPLY confirmation token by itself must not manufacture a Human Gate.

Codex provider fallback: on Windows prefer the standalone global npm Codex provider first, then the VS Code bundled codex.exe fallback. Only actual exhaustion of both providers, or token/capacity unavailability, may switch the durable Shoestring GOAL executor mode to web-gpt-ds. Use the closed Shoestring codex-failure transition so Telegram is attempted before the mode switch. A single task-local stall must never advance the provider chain.

Mandatory TG stop rule: any reason that causes forward execution to stop must proactively notify the Owner through Telegram before yielding or waiting. This includes tool/runtime/Bridge/DS/Codex failures, no-safe-path conditions, unresolved material ambiguity, authorization boundaries, owner-only decision waits, and Owner-requested pause/stop. For a recoverable system interruption, use the shared Shoestring GOAL interruption path, notify first, then recover and continue automatically when safe. An explicit Owner-requested pause/stop uses the dedicated PAUSED_BY_OWNER pause notification instead of a system INTERRUPTED event, but it is still mandatory TG.
