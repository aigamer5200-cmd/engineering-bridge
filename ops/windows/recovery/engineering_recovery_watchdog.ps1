...
Codex provider fallback: on Windows prefer the standalone global npm Codex provider first, then the VS Code bundled codex.exe fallback. If the Codex provider chain is exhausted or Codex remains unavailable, emit the required Telegram interruption notice, durably switch the Shoestring GOAL executor mode to web-gpt-ds, and continue the same frozen GOAL with Web GPT + DS. Do not stop merely because Codex is unavailable when DS remains a safe authorized executor path.
