...
- `run_task` starts Codex through `codex app-server --stdio` without a shell. Owner-approved Codex tasks default to `danger-full-access` and may be explicitly narrowed to `workspace-write` or `read-only`; the exact sandbox is persisted in execution provenance. DSH remains pinned to `DSH_PERMISSION_MODE=read-only` and rejects sandbox expansion. Controlled-patch generation/refinement remain read-only. DSH receives only its explicit environment allowlist and no proxy variables.
...
- Controlled-patch APPLY access defaults to disabled. Manual workspaces enable that path through `allow_write: true`; managed workspaces grant it through exact `AUTHORIZE`. This gate is separate from the Owner-approved Codex `run_task` sandbox.
...
Codex `run_task` may intentionally run with `danger-full-access`, so the trusted local operator must treat the natural-language instruction as code-execution authority available to the Codex process. Callers can narrow individual Codex tasks when appropriate. DSH and controlled-patch proposal generation remain read-only. None of these modes creates an additional OS-level read jail beyond Codex/host controls.
